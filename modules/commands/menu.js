...
module.exports.run = async function({ api, event, args }) {
    ...
    if (args.length >= 1) {
      ...
      var txt = '\n',count = 0;
      for (const {commandCategory,commandsName} of data) txt += `${++count}. ${commandCategory}\n→ Tên lệnh: ${commandsName}\n`;
      txt += `😻 Bot hiện tại có ${global.client.commands.size} lệnh\n Reply từ 1 đến ${data.length} để chọn\n🌐 Facebook: ${(!global.config.FACEBOOK_ADMIN) ?  "Thêm facebook admin ở config!" : global.config.FACEBOOK_ADMIN}`;
      const msg = txt;
      send(msg, tid, (a, b) => {
